...
function setup() {
  ...
  // Generate the Perlin noise and play sound for the entire grid
  generateNoiseGrid();
}

function keyPressed() {
  // Start playing sound when any key is pressed
  playSound();
}
...
function playSound() {
  let gridSize = 3;
  let padding = 20;
  let cellSize = (min(width, height) - padding * (gridSize - 1)) / gridSize;
  let duration = 0.5; // Duration of the sound in seconds
      ...
      for (let x = 0; x < cellSize; x++) {
        for (let y = 0; y < cellSize; y++) {
          ...
          if (actualX < width && actualY < height) {
            ...
            // Add the frequency to the audio data array
